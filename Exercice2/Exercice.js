function moy() {
  ...
  if (parseFloat(($('#grade1').val())) < 0 || parseFloat(($('#grade1').val())) < 0 || parseFloat(($('#grade3').val())) < 0 || parseFloat(($('#grade4').val())) < 0 || parseFloat(($('#grade5').val())) < 0) {
    ...
    $('#reset').removeClass('d-none');
  } else if(isNaN($('#grade1').val()) || isNaN($('#grade2').val()) || isNaN($('#grade3').val()) || isNaN($('#grade4').val()) || isNaN($('#grade5').val())){
    $('#gradeForm').addClass('d-none');
    $('#tooMuch').text(function() {
      return 'Vraiment??? Des lettres? Non mais t\'as bu?? Allez, recommence !!'
    });
    $('#reset').removeClass('d-none');
  }else if(($('#grade1').val().length==0) || ($('#grade2').val().length==0) || ($('#grade3').val().length==0) || ($('#grade4').val().length==0) || ($('#grade5').val().length==0)){
    $('#gradeForm').addClass('d-none');
    $('#tooMuch').text(function() {
      return 'Euh... On est d\'accord que tu as pas vraiment tout remplis la non? Allez, recommence!!'
    });
    $('#reset').removeClass('d-none');
  }else if(($('#firstName').val().length==0) || ($('#lastName').val().length==0)){
    $('#gradeForm').addClass('d-none');
    $('#tooMuch').text(function() {
      return 'Il a ptet un prénom et un nom l\'élève non? Allez, recommence !!'
    });
    $('#reset').removeClass('d-none');
  }else{
